...
{
	var kmeans = [];
	var min = array.reduce((acc, val) => (val < acc ? val : acc), Number.MAX_VALUE);
	var max = array.reduce((acc, val) => (val > acc ? val : acc), Number.MIN_VALUE);
	var step = (max - min) / means;
	for(var i=0;i < means;i++)
	{
		var cord = {value:(i * ((max - min) / means) + min),sum:0,pre:0,points:[]};
		kmeans.push(cord);
	}
	return kmeans;
}
